...
<script>
  import Preview from './applicationCard/Preview';
  import Edit from './applicationCard/Edit';
  ...
  export default {
    ...
    created() {
      if (['idle', 'rejected'].includes(this.applicationMethod.status)) {
        this.fetchApplicationMethod();
      }
      if (['idle', 'rejected'].includes(this.applicationTimeCost.status)) {
        this.fetchApplicationTimeCost();
      }
    },
    methods: {
      ...canvasMapActions(['fetchApplicationMethod', 'fetchApplicationTimeCost']),
