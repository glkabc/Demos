<template>
  <div class="qualityControl">
    ...
    <div class="quality-control">
      <div class="title">
        <a-row class="header-show">
          <a-col :span="10">
            <span class="left-arrow">
              <span class="left-route">{{ listName }}</span>
              <span style="padding:0 6px;">|</span>
            </span>
            <span class="route no-select">画布清单</span>
          </a-col>
          <a-col :span="12" class="card-container-ch">
            <a-tabs
              type="card"
              :default-active-key="defaultActiveTabNum"
              @change="getTab"
            >
              <a-tab-pane key="2">
                <span slot="tab">
              ...
              <a-tab-pane key="3" force-render>
                <span slot="tab">
                  <i
                    class="iconfont icon-yipingshen-xuanzhong"
                    style="font-weight:normal;"
                  ></i>
                  已审核
                </span>
        ...
        <div class="container" v-if="listData.length > 0">
          ...
          >
            ...
            <div class="top-box">
              ...
              <span
                >课堂画布《
                <span class="class-name-contain-box"
                  ><a-tooltip placement="right">
                    <template slot="title" v-if="isShowName">
                      {{ item.aclass.name }}
                    </template>
                    <div
                      class="add-item-aclass-name"
                      ref="classNameBoxRef"
                      @mouseenter="handleShowName(item.aclass.id)"
              ...
              <!-- 审核中... -->
              <span
                v-if="defaultActiveTabNum == 2"
                class="top-box-status2"
                style="color:#046E72"
              >
                {{ item.aclass.createUserName }}老师
              </span>
              <!-- 审核通过老师名称、分数及评价 -->
              <span
                v-else
                class="top-box-status3"
                :style="activation(item.aclass)"
              >
                {{ item.aclass.createUserName }}老师 / {{ item.aclass.score }}分
                {{ item.aclass.scoreEvaluate }}&nbsp;
              </span>
            </div>
            <!-- 画布展示 -->
            <div class="flexs-box">
              ...
              <div class="capsule-btn-box">
                <div class="btn-box">
                  <a-button
                    class="check-btn"
                    v-if="defaultActiveTabNum == 2"
                    @click="handleCheck(item.mainTask.id)"
                  >
                  ...
                  <a-button
                    class="check-btn"
                    v-if="defaultActiveTabNum == 3"
                    @click="handlePreview(item.mainTask.id)"
                  >
...
<script>
  ...
  const {mapActions: canvasMapActions} = createNamespacedHelpers('canvas');
  ...
  export default {
    ...
    data() {
      return {
        ...
        loadingEnd: false,
        listName: this.$route.query.status==3?'已审核列表':'审核中列表',
        defaultActiveTabNum: this.$route.query.status
    ...
    methods: {
      ...
      // 显示tooltip
      handleShowName(id) {
        const matcher = this.$refs.classNameBoxRef.find(
      ...
      async getClassListInfo() {
        ...
        obj.status = Number(this.defaultActiveTabNum);
        // 获取页面展示的数据
        const res = await getClassListInfo(obj);
        if (res.data.code == 0) {
          ...
          for (let i = 0; i < res.data.data.list.length; i++) {
            ...
            let averageObj = {};
            for (let j = 0; j < res.data.data.list[i].capsuleList.length; j++) {
              let obj = {};
              obj.subTask =
                res.data.data.list[i].capsuleList[j].subTask.average;
              obj.secondaryCard =
                res.data.data.list[i].capsuleList[j].secondaryCard.endTime;
              if (
                res.data.data.list[i].capsuleList[j].secondaryCard.endTime ==
                res.data.data.list[i].capsuleList[j].secondaryCard.beginTime
              ) {
                obj.secondaryCard = 0;
              } else {
              ...
              for (
                let y = 0;
                y < res.data.data.list[i].capsuleList[j].groupList.length;
                y++
              ) {
                for (
                  let q = 0;
                  q <
                  res.data.data.list[i].capsuleList[j].groupList[y]
                  ...
                  (activationCardData.introContent =
                    res.data.data.list[i].capsuleList[j].groupList[
                      y
                    ].activationCard[q].relatedKnowledge),
                    (activationCardData.introFormat =
                      res.data.data.list[i].capsuleList[j].groupList[
                        y
                      ].activationCard[q].activationTypeId),
                    (activationCardData.timeCost =
                      res.data.data.list[i].capsuleList[j].groupList[
                        y
                      ].activationCard[q].dictionaryExcepttimeId),
                    (activationCardData.activityContent =
                      res.data.data.list[i].capsuleList[j].groupList[
                        y
                      ].activationCard[q].content),
                ...
                for (
                  ...
                  (applicationCardData.how =
                    res.data.data.list[i].capsuleList[j].groupList[
                      y
                    ].applicationCard[w].practiceId),
                    (applicationCardData.timeCost =
                      res.data.data.list[i].capsuleList[j].groupList[
                        y
                      ].applicationCard[w].dictionaryExcepttimeId),
                    (applicationCardData.description =
                      res.data.data.list[i].capsuleList[j].groupList[
                        y
                      ].applicationCard[w].content),
                    (applicationCardData.id =
                      res.data.data.list[i].capsuleList[j].groupList[
                        y
                      ].applicationCard[w].id),
                ...
                for (
                  ...
                  (demonstrationCardData.knowledge =
                    res.data.data.list[i].capsuleList[j].groupList[
                      y
                    ].demonstrationCard[e].knowledgePoint),
                    (demonstrationCardData.how =
                      res.data.data.list[i].capsuleList[j].groupList[
                        y
                      ].demonstrationCard[e].demonstrationTypeid),
                    (demonstrationCardData.timeCost =
                      res.data.data.list[i].capsuleList[j].groupList[
                        y
                      ].demonstrationCard[e].dictionaryExcepttimeId),
                    (demonstrationCardData.description =
                      res.data.data.list[i].capsuleList[j].groupList[
                        y
                      ].demonstrationCard[e].displayCase),
                    (demonstrationCardData.termExplanation =
                      res.data.data.list[i].capsuleList[j].groupList[
                        y
                      ].demonstrationCard[e].popularDescription),
                    (demonstrationCardData.id =
                      res.data.data.list[i].capsuleList[j].groupList[
                        y
                      ].demonstrationCard[e].id),
      ...
      handleFormData() {},
      handleCheck(id) {
        const checkFormData = this.listData.find((item) => {
          return item.mainTask.id === id;
        });
        this.$router.push({
          path: '/quality/score',
          query: {status: 2, classId: checkFormData.aclass.id},
        });
      },
      handlePreview(id) {
        const checkFormData = this.listData.find((item) => {
        ...
        this.$router.push({
          ...
          query: {status: 3, classId: checkFormData.aclass.id},
        });
      },
      // 自定义分页
      ...
      getMainTaskOriginFormData(cardId) {
        // mainTaskFormData 中存储所有的主任务卡用户填写的数据
...
<style scoped>
  ...
  /deep/ .card-container-ch .ant-tabs-tab {
  ...
  /deep/ .capsule-btn-box .btn-box .ant-btn-primary {
    background: #357981;
    color: #fff;
  }
  /deep/ .card-container-ch .ant-tabs-nav .ant-tabs-tab .anticon {
    margin-right: 0;
  }
