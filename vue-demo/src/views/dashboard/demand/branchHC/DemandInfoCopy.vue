<template>
  <div class="demand-content-container">
    <a-form :form="staffForm" @submit="handleOrganSubmit" :forceRender="true">
      ...
      <a-row class="add-demand-content">
        ...
        <!-- <a-col :span="1"
          ><IconFont
            ...
            @click="showIntervier"
            class="icon"
            style="font-size:25px;height:32px;margin-left:-30px;line-height:40px;color:#38bdd3;"
        /></a-col>
        <a-col
          :span="18"
          :offset="4"
    ...
    <a-affix :offsetBottom="0">
      <div class="bottom">
        ...
        <!-- <a-button
          ...
          class="bottom-btn"
          style="margin-right:10px;"
          @click="handleOrganSubmit(2)"
          v-if="this.$route.query.status != 2"
          >保存并复制</a-button
        >
        <a-button
          type="primary"
...
export default {
  ...
  methods: {
    ...
    async getDemandById() {
      try {
        ...
        this.echoList.hc_department =
          res.data.data.demand.organizeIdList !== null
            ? res.data.data.demand.organizeIdList
            : [];
    ...
    selectOrgansize(value) {
        ...
        console.log(value)
        getAllCustomOrg(value).then(res => {
    ...
    handleOrganSubmit(index) {
      this.staffForm.validateFields((error, fieldsValue) => {
        ...
        console.log(values.hc_department)
        // let sssss = this.departmentList.map(val => {
        //   console.log(val)
        // })
        if (this.isShowError) {
        ...
        // console.log(this.listQuery.organizeId)
        ...
        this.listQuery.hc_department=values.hc_department;
        // this.listQuery.demandId = Number(this.$route.query.demandId);
        console.log("Received values of form: ",this.echoList.jobTypeName, values);
        if (index == 4) {
          //发布
            ...
            console.log()
              ...
              addDemand(this.listQuery).then((res) => {
                if (res.data.code === 0) {
                  ...
                  this.$message.error(res.data.mesg || "复制需求失败", 2);
                }
              });
