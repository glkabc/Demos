<template>
  <div class="container" v-if="message.list.length > 0">
      <AddClass :typeCreate='2' @addClassItemOne="addClassItemOne" v-show="meta.pageNum === 1"/>
      <ClassItem :typeCreate='2' @addClassItemOne="addClassItemOne"  v-for="(item,index) in message.list" :item='item' :key="index"/>
      <!-- <ADDBOX/> -->
      ...
      getMessage() {
        if (this.getDatasLodaing && this.getNextPageData) {
            ...
            Axios.post('/course/getCourseList', {
                // query: {
                    createType: this.$route.params.type == null ? 2 : this.$route.params.type, //（1.我创作的 2.我实战的）
                ...
                this.getStatus = err.response.status.tostring
                // alert('数据请求失败')
            })
        } else {
