...
const routes = [
  {
    path: '/loginme',
    name: '登录',
    component: () => import(/* webpackChunkName: "loginme", webpackPrefetch: true */ '../views/writ/Login'),
  },
  // {
  //   path: '/loginme',
  //   name: '我的登录',
  //   component: () => import('../views/writ/Login')
  // },
  {
    path: '/',
    name: '工作台',
...
router.beforeEach((to, _, next) => {
  // 跳转的路由是否需要保护
    ...
    // if(to.meta.protected){
      ...
      next();
			// if(to.path=='/loginme'||to.path=='/loginbs'||to.path=='/noAuth') {
      //       next()
      //     }
      //     else{
      //       next({ name: '登录' });
